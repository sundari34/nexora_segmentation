...
func EvaluteRaw(req models.SegmentNewPayload) (map[string]interface{}, error) {
	...
	if len(groupWhere) > 0 {
		...
		withStatement = fmt.Sprintf("WITH event_users AS (SELECT DISTINCT ev.nexora_id FROM events ev INNER JOIN event_daily ed ON ev.event_name = ed.event_name AND ev.nexora_id = ed.nexora_id %s), cp_resolved AS (SELECT cp.id, argMaxMerge(cp.nexora_id_state) AS nexora_id, argMaxMerge(cp.email_state) AS email, argMaxMerge(cp.name_state) AS name, argMaxMerge(cp.mobile_state) AS mobile, argMaxMerge(cp.updated_at_state) AS updated_at, argMaxMerge(cp.project_id_state) AS project_id FROM customer_profiles_latest cp GROUP BY cp.id)", finalWhere)
