...
func EvaluteRaw(req models.SegmentNewPayload) (map[string]interface{}, error) {
	...
	if req.Source == "campaign_service" {
		selectStatement = fmt.Sprintf("SELECT any(np.nexora_id) AS nexora_id, coalesce( nullIf(JSONExtractString(argMaxMerge(cp.user_properties_state), '%s'), ''), 'default') AS property", req.Property)
	}
	// check fot where
	if len(groupWhere) > 0 {
