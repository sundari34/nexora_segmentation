...
func EvaluteRaw(req models.SegmentNewPayload) (map[string]interface{}, error) {
	...
	selectStatement := "SELECT argMaxMerge(cp.id) AS customer_profile_id, argMaxMerge(cp.nexora_id_state) AS nexora_id, argMaxMerge(cp.email_state) AS email, argMaxMerge(cp.mobile_state) AS mobile, argMaxMerge(cp.name_state) AS name, argMaxMerge(cp.project_id_state) AS project_id, argMaxMerge(cp.client_id_state) AS client_id, argMaxMerge(cp.user_properties_state) AS user_properties, JSONExtractString(argMaxMerge(cp.user_properties_state), 'gender') AS gender"
	if req.Source == "campaign_service" {
		selectStatement = fmt.Sprintf("SELECT argMaxMerge(cp.id) AS customer_profile_id, argMaxMerge(cp.nexora_id_state) AS nexora_id, argMaxMerge(cp.email_state) AS email, argMaxMerge(cp.mobile_state) AS mobile, argMaxMerge(cp.name_state) AS name, argMaxMerge(cp.project_id_state) AS project_id, argMaxMerge(cp.user_properties_state) AS user_properties, coalesce( nullIf(JSONExtractString(argMaxMerge(cp.user_properties_state), '%s'), ''), 'default') AS property", req.Property)
	}
	...
	if len(groupWhere) > 0 {
		...
		overallSelectStatement = fmt.Sprintf("%s, cp_resolved AS (%s from customer_profiles_latest cp %s group by argMaxMerge(cp.nexora_id_state) %s) select * from %s order by argMaxMerge(cp.nexora_id_state) %s", withStatement, selectStatement, whereNonAggregateStatement, finalHaving, joinStatement, limitAndOffsets)
		countStatement = fmt.Sprintf("%s, cp_resolved AS (%s from customer_profiles_latest cp %s group by argMaxMerge(cp.nexora_id_state) %s) select COUNT(*) AS total_count FROM %s order by argMaxMerge(cp.nexora_id_state) %s", withStatement, selectStatement, whereNonAggregateStatement, finalHaving, joinStatement, limitAndOffsets)

	} else {
		joinStatement = "customer_profiles_latest cp"
		overallSelectStatement = fmt.Sprintf("%s from %s %s group by argMaxMerge(cp.nexora_id_state) %s order by argMaxMerge(cp.nexora_id_state) %s", selectStatement, joinStatement, whereNonAggregateStatement, finalHaving, limitAndOffsets)
		countStatement = fmt.Sprintf("SELECT COUNT(*) AS total_count FROM ( %s from %s %s group by argMaxMerge(cp.nexora_id_state) %s order by argMaxMerge(cp.nexora_id_state) %s) as sub", selectStatement, joinStatement, whereNonAggregateStatement, finalHaving, limitAndOffsets)
	}
